...
filename = 'LCEDA-' + sourcename.split('.', 1)[0] + '-' + datetime.datetime.now().__format__("%Y-%m-%d-%H-%M-%S") + '.json';#目标文件名
#layer = 7;#所在层
threshold = 127;#阈值
#单位转换
    ...
    for j in range(im_col):
            ...
            if (((i / img.shape[0])*100) - report_f) > 0.01:
                if report_f%10 == 0:
                    ...
                    print('|  %2.0f%%\t' % ((i / img.shape[0])*100),  flush=True, end = '')
                report_f = report_f + 1;
print('| 100%\t|')
#读写文件
...
for i in range(lines_mil.shape[0]):
    ...
    if (((i / lines_mil.shape[0])*100) - report_f) > 0.01:
        if report_f%10 == 0:
            print('|  %2.0f%%\t|' % ((i / lines_mil.shape[0])*100),  flush=True) 
        else:
            print('|  %2.0f%%\t' % ((i / lines_mil.shape[0])*100),  flush=True, end = '') 
        report_f = report_f + 1;
print('| 100%\t|')
#写入文件内容
